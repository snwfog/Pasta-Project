...
class ScheduleBuilder_model extends CI_Model {
    function filter_courses_by_preference($courses, $time, $longWeekend, $season) {
        ...
        foreach($courses as $key => $course):
            $lectures = $this->db->query("SELECT lectures.id, section, professor, season, course_id, time_location_id, start_time, end_time, day
            ...
            AND time_locations.start_time" . $constraint . " " . $fridayOff)->result_array();
    ...
    private function check_tutorials($lectures, $constraint) {
        ...
        foreach($lectures as $key => $lecture):
            ...
            $tutorials = $this->db->query("SELECT tutorials.id, tutorials.section, lecture_id, tutorials.time_location_id, start_time, end_time, day
                                ...
                                and time_locations.start_time" . $constraint)->result_array();
    ...
    private function check_labs($tutorials, $constraint) {
        ...
        foreach($tutorials as $key => $tutorial):
            ...
            $labs = $this->db->query("SELECT labs.id, labs.section, tutorial_id, labs.time_location_id, start_time, end_time, day
            ...
            foreach($B as $b) {
                ...
                if (!empty($same_days)) {
                    if ($b["start_time"] > $a["start_time"] && $b["start_time"] < $a["end_time"]) {
                        return true;
                    } elseif ($b["start_time"] < $a["start_time"] && $b["end_time"] > $a["start_time"]) {
                        return true;
    ...
    function sort_courses_by_type($courses) {
        ...
        $sorted_courses = array("Core" => array(), "Basic Sciences" => array(), "General Electives" => array(), "Technical Electives" => array(), "Others" => array());
